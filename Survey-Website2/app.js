...
app.route("/password")
  ...
  .post(async (req, res) => {
    if (!req.body.password || !req.body.passwordConfirm || /[^A-Za-z0-9]/.test(req.body.password) || /[^A-Za-z0-9]/.test(req.body.passwordConfirm)) {
      return res.render("password", { msg: "Invalid password, Letters and numbers only" })
    }
...
app.route("/survey")
  ...
  .post(async (req, res) => {
    ...
    // invalid answers
    const surveyQs = require("./misc/surveys.json")
    const aArray = []
    for (var surveyQ of surveyQs) {
      for(var option of surveyQ.options){
        aArray.push(option)
      }
    }
    for (var key in req.body) {
      if (!aArray.includes(req.body[key])) {
        return res.redirect("/survey")
      }
    }
